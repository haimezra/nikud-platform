(function() {
  // הערה: ההגדרות הן per-page-load בלבד (לא נשמרות בין רענונים/ביקורים).
  // זה מכוון - עמדת צ'ק-אין היא לרוב מכשיר משותף בין כמה עובדים, ולא
  // רצוי שהגדרת נגישות של עובד אחד "תידבק" לעובד הבא שמשתמש במכשיר.
  var state = {
    fontScale: 1,
    contrast: false,
    grayscale: false,
  ...
  document.getElementById('a11y-contrast').onclick = function() { state.contrast = !state.contrast; applyAll() }
  document.getElementById('a11y-grayscale').onclick = function() { state.grayscale = !state.grayscale; applyAll() }
  document.getElementById('a11y-underline').onclick = function() { state.underlineLinks = !state.underlineLinks; applyAll() }
  document.getElementById('a11y-stopanim').onclick = function() { state.stopAnimations = !state.stopAnimations; applyAll() }
  document.getElementById('a11y-cursor').onclick = function() { state.bigCursor = !state.bigCursor; applyAll() }
  document.getElementById('a11y-readable').onclick = function() { state.readableFont = !state.readableFont; applyAll() }
